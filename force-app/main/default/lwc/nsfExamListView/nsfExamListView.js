...
import getExamSectionSummaries from '@salesforce/apex/NSF_ContentController.getExamSectionSummaries';
...
export default class NsfExamListView extends LightningElement {
    @api notebookId = '';
    ...
    @wire(getExamSectionSummaries, { notebookId: '$notebookId' })
    wiredSummaries(result) {
        this._wiredSummariesResult = result;
        if (result.data) {
            this._totalExams = result.data.reduce(
            ...
            this.sectionSummaries = result.data.map(s => ({
                ...
                countLabel: `Total questions in exam: ${s.itemCount || 0}`,
        ...
        if (result.error) {
            this.showError('Error loading exam sections', result.error);
        }
    }

    @wire(getBookmarkedItems, { notebookId: '$notebookId', mode: 'Exams' })
    wiredBookmarks(result) {
        this._wiredBookmarksResult = result;
        if (result.data && result.data.length > 0) {
            this.bookmarkSummary = {
                itemCount: result.data.length,
                countLabel: `Total questions in exam: ${result.data.length}`,
                dateLabel: ''
            };
        } else {
            this.bookmarkSummary = {
                itemCount: 0,
                countLabel: 'Total questions in exam: 0',
                dateLabel: ''
            };
        }
        if (result.error) {
    ...
    handleOpenUpsert(event) {
        this.upsertExamId = event.detail.examId || null;
    ...
    _calcPercentage(count) {
        if (!this._totalExams || !count) return '';
        return Math.round((count / this._totalExams) * 100) + '%';
    }
